...
func TestConnectWithRetryOverflow(t *testing.T) {
	d := newTestDialer()
	...
	f := &Fluent{
		Config: Config{
			...
			FluentHost:    "localhost",
			FluentPort:    24224,
			RetryWait:     math.MaxInt32 / 2, // 큰 값을 사용하여 오버플로우 유도
			MaxRetryWait:  60000,
			MaxRetry:      3,
		},
		dialer: d,
	...
	go func() {
		errCh <- f.connectWithRetry(context.Background())
	}()

	// 1초 안에 종료되지 않으면 hang으로 간주
	select {
	case err := <-errCh:
		// 정상적으로 에러를 반환하면 성공
		if err == nil {
